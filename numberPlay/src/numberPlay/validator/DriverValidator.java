	...
	private static class ValidatorFetcher {
		...
		public static Validator argsValidator(DriverValidator d) {
			return new Validator() {
				@Override
				public void run() throws Exception {
					if ((d.getArgs()[0].equals("${inputNumStream}")) || (d.getArgs()[1].equals("${runAvgWindowSize}"))
							|| (d.getArgs()[2].equals("${runAvgOutFile}") ) || (d.getArgs()[3].equals("${k}") ) ||(d.getArgs()[4].equals("${topKNumOutFile}") ) 
							|| (d.getArgs()[5].equals("${numPeaksOutFile}") )) {
